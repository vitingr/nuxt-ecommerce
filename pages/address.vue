<template>
  <main-layout>
    <div id="AddressPage" class="mt-4 max-w-[500px] mx-auto px-2 pt-10">
      <div class="mx-auto bg-white rounded-lg p-6">
        ...
        <form @submit.prevent="submit()">
          ...
          <button
            :disabled="isWorking"
            type="submit"
            class="mt-10 bg-gradient-to-r from-[#FE630C] to-[#FF3200] w-full text-white text-[21px] font-semibold p-1.5 rounded-full"
          >
            <div v-if="!isWorking" class="text-white">Update Address</div>
...
watchEffect(async () => {
  ...
  if (currentAddress.value.data) {
    let contactName = currentAddress.value.data.name;
    let address = currentAddress.value.data.address;
    let zipCode = currentAddress.value.data.zipcode;
    let city = currentAddress.value.data.city;
    let country = currentAddress.value.data.country;
